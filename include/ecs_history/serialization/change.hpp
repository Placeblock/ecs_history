...
class change_serializer final : public any_change_supplier_t {
public:
    explicit change_serializer(Archive &archive)
        : archive(archive) {
    }

    void apply_construct(static_entity_t static_entity,
    ...
    void apply_destruct(static_entity_t static_entity,
...
std::unique_ptr<change_t<Type> > deserialize_change(Archive &archive) {
    ...
    case change_type_t::CONSTRUCT: {
        ...
        return std::make_unique<construct_change_t<Type> >(
            static_entity,
            value.cast<Type>());
    }
    case change_type_t::UPDATE: {
        ...
        return std::make_unique<update_change_t<Type> >(
            static_entity,
            old_value.cast<Type>(),
            new_value.cast<Type>());
    }
    ...
    case change_type_t::DESTRUCT: {
        ...
        serialize_component<Archive, false>(archive, old_value);
        return std::make_unique<destruct_change_t<Type> >(
            static_entity,
            old_value.cast<Type>());
    }
