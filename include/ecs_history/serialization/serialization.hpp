...
void serialize_commit(Archive &archive, commit_t &commit) {
    ...
    for (const static_entity_t &deleted_entity : commit.destroyed_entities) {
...
template<typename Archive>
std::vector<static_entity_t> deserialize_entity_list(Archive &archive) {
    uint32_t entity_count;
    archive(entity_count);
    auto static_entities = std::vector<static_entity_t>(entity_count);
